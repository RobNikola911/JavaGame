...
public class PauseMenu extends State {
  ...
  public PauseMenu(Handler handler) {
    ...
    this.pauseManager.addObject(
        new UIImageButton(900, 150, 52, 52,
            Assets.getMenuElement("sound"),
            new ClickListener() {
          @Override
          public void onClick() {
            handler.getGame().setMuted(!handler.getGame().isMuted());
            Assets.getMenuElement("sound")[0] =
                Assets.getMenuElement("soundAlt")[0];
            Assets.getMenuElement("sound")[1] =
                Assets.getMenuElement("soundAlt")[1];
            Assets.getMenuElement("soundAlt")[1] =
                Assets.getMenuElement("sound")[0];
            Assets.getMenuElement("soundAlt")[0] =
                Assets.getMenuElement("sound")[1];
    ...
    this.pauseManager.addObject(
        new UIImageButton(800, 50, 159, 107,
            Assets.getMenuElement("btn_quit"),
            new ClickListener() {
              @Override
              public void onClick() {
                try {
                  Save.exportSave(
                      /*PauseMenu.super.getHandler().getWorld().getEntityManager().getPlayer()
