...
public abstract class Villain extends Creature {
  ...
  Villain(Handler handler, float x, float y, int width, int height) {
    ...
    if (this instanceof ChaserVillain) {
      this.animLeft = new Animation(DEFAULT_VILLAIN_SPEED,
          Assets.getEntitiesMotionPositions("chaserVillain_Left"));
      this.animRight = new Animation(DEFAULT_VILLAIN_SPEED,
          Assets.getEntitiesMotionPositions("chaserVillain_Right"));
      this.animUp = new Animation(DEFAULT_VILLAIN_SPEED,
          Assets.getEntitiesMotionPositions("chaserVillain_Up"));
      this.animDown = new Animation(DEFAULT_VILLAIN_SPEED,
          Assets.getEntitiesMotionPositions("chaserVillain_Down"));
    } else {
      this.animLeft = new Animation(DEFAULT_VILLAIN_SPEED,
          Assets.getEntitiesMotionPositions("villain_Left"));
      this.animRight = new Animation(DEFAULT_VILLAIN_SPEED,
          Assets.getEntitiesMotionPositions("villain_Right"));
      this.animUp = new Animation(DEFAULT_VILLAIN_SPEED,
          Assets.getEntitiesMotionPositions("villain_Up"));
      this.animDown = new Animation(DEFAULT_VILLAIN_SPEED,
          Assets.getEntitiesMotionPositions("villain_Down"));
    }
  }
  ...
  void checkAttacks() {
    ...
    Rectangle collisionBounds = getCollisionBounds(this.getInitialCollisionOffsets()[0],
        this.getInitialCollisionOffsets()[1]);
    ...
    if (super.getHandler().getWorld().getEntityManager().getPlayer()
        .getCollisionBounds(this.getInitialCollisionOffsets()[0],
            this.getInitialCollisionOffsets()[1]).intersects(attackBounds)) {
