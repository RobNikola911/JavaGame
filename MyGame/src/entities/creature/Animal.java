...
public class Animal extends Creature {
  ...
  public Animal(Handler handler, float x, float y) {
    ...
    this.animLeft = new Animation(ANIMATION_SPEED,
        Assets.getEntitiesMotionPositions("sheep_Left"));
    this.animRight = new Animation(ANIMATION_SPEED,
        Assets.getEntitiesMotionPositions("sheep_Right"));
    this.animUp = new Animation(ANIMATION_SPEED,
        Assets.getEntitiesMotionPositions("sheep_Up"));
    this.animDown = new Animation(ANIMATION_SPEED,
        Assets.getEntitiesMotionPositions("sheep_Down"));
  ...
  private void time() {
    ...
    if (System.nanoTime() % (random.nextInt(super.getRandomDivisorBound()) + super
        .getDirectionChangeDelay()) == 0) {
      moveSheep();
    }
  }
  ...
  private BufferedImage getCurrentAnimationFrame() {
    if (this.getxMove() < 0) {
      ...
      switch (this.lastMovedDirection) {
        ...
        case "Left":
          return Assets.getEntitiesStillPositions("sheep_LeftStill");
        case "Right":
...
/*  @Override
  public void die() {
    super.getHandler().getWorld().getItemManager().addItem(
        ItemFactory.createNew("meat", (int) super.getX(), (int) super.getY(), super.getHandler()));
  }*/
}
